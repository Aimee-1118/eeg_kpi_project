...
1.  **M1 (Load):** `data_raw/` 폴더의 CSV 원본 데이터를 MNE 객체로 로드합니다. (EEG+STIM 채널)
2.  **M2 (Preprocess):** 노치 필터(예: 60Hz) 및 대역통과 필터(예: 0.5-40Hz)를 적용합니다. (핵심 노이즈 제거 1)
3.  **M3 (ICA):** (옵션) `base_config.yaml`의 `USE_ICA=True`일 때만 ICA를 실행합니다. (2채널 환경에서는 권장되지 않음)
4.  **M4 (Epoch):** 정제된 데이터를 `EVENT_IDS` 기준 블록으로 나누고, `EPOCH_DURATION_SEC` 단위(예: 5초)로 분할합니다. `REJECT_THRESHOLD_UV`를 초과하는 아티팩트 Epoch를 제거합니다. (핵심 노이즈 제거 2)
5.  **M5 (Extract):** 본 프로젝트의 핵심 CPU. `features/` 폴더의 함수를 호출하여 A, B, C 카테고리의 모든 KPI와 숫자 라벨(`1` 또는 `2`)을 추출합니다.
...
프로젝트 실행에 필요한 라이브러리 목록이 `requirements.txt` 파일에 포함되어 있습니다. (OmegaConf, fooof 포함)

...
프로젝트는 6개의 핵심 모듈로 구성된 자동화 파이프라인을 따릅니다.

1.  **M1 (Load):** `data_raw/` 폴더의 CSV 원본 데이터를 MNE 객체로 로드합니다.
2.  **M2 (Preprocess):** 노치 필터 및 대역통과 필터를 적용합니다.
3.  **M3 (ICA):** ICA(독립성분분석)를 실행하여 눈 깜빡임(EOG) 등 핵심 노이즈를 자동으로 제거합니다.
4.  **M4 (Epoch):** 정제된 데이터를 분석 단위(Epoch)로 분할하고, 아티팩트가 포함된 Epoch를 제거합니다.
...
본 파이프라인은 '바텀업' 접근을 위해 다음 세 가지 대분류의 모든 지표를 추출합니다.
  ...
  * **A. 형태학적 & 시간축 변수:** 파형의 원본(x(t)) 신호의 '모양', '타이밍', '통계적 특성' (예: 진폭, 잠복기, Hjorth 파라미터, 1\~4차 모멘트)
...
1.  **데이터 준비:** 분석할 EEG 원본 파일(`.csv`)을 `data_raw/` 폴더 안에 넣습니다.

2.  **설정:** `config.py` 파일을 열어 `CHANNELS` (채널 이름), `SAMPLE_RATE` (샘플링 레이트), 필터 값 등을 자신의 데이터에 맞게 수정합니다.
...
4.  **결과 확인:** 파이프라인이 완료되면 `results/` 폴더에 `final_kpi_table.csv` 파일이 생성됩니다. 이 파일을 R, SPSS 또는 Python(Scikit-learn)에서 불러와 최종 통계/머신러닝 분석을 수행하면 됩니다.
...
# (🔥 ICA를 옵션으로 변경하는 'USE_ICA' 플래그 추가됨)
...
# (🔥 신규) M3 ICA 사용 여부 (2채널에서는 False 권장)
# True로 설정하면 M3(ICA) 모듈이 실행됩니다.
# False로 설정하면 M3 모듈을 건너뛰고,
# M2(필터링) + M4(임계치 제거)로만 노이즈를 처리합니다.
USE_ICA = False  

# 👁️ (ICA=True일 경우) EOG(눈 깜빡임) 탐지용 채널
EOG_CHANNEL_NAME = 'Fp1'
# 🔢 (ICA=True일 경우) ICA 재현성을 위한 Random State
ICA_RANDOM_STATE = 97
...
# B. 주파수축: Welch PSD 계산 시 윈도우 길이 (초)
WELCH_WINDOW_SEC = 2.0
# B. 주파수축: 1/f 지수(기울기) 계산 시 사용할 주파수 범위 (Hz)
...
# C. 동역학: 알파 버스트 임계값 (평균 + N * 표준편차)
ALPHA_BURST_THRESHOLD_SD = 1.0
# C. 동역학: 파워 변동성(Spectrogram) 계산 시 윈도우 길이 (초)
POWER_VAR_WINDOW_SEC = 1.0
# C. 동역학: 파워 변동성 계산 시 윈도우 중첩 비율 (0.0 ~ 1.0)
POWER_VAR_OVERLAP_RATIO = 0.5  # 50% 중첩

# C. 연결성: Coherence, PLV, wPLI 계산 시 윈도우 길이 (초)
CONN_WINDOW_SEC = 2.0